...
export function isRustNotebookCell(document: vscode.TextDocument): document is RustDocument {
    console.log(document)
    // Prevent corrupted text (particularly via inlay hints) in diff views
    // by allowing only `file` schemes
    // unfortunately extensions that use diff views not always set this
    // to something different than 'file' (see ongoing bug: #4608)
    return document.languageId === 'rust' && document.uri.scheme === 'vscode-notebook-cell';
}
